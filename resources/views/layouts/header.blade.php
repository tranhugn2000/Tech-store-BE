<header class="z-10 py-[22px] bg-white shadow-md dark:bg-gray-800">
  <div class="container flex items-center justify-between h-full px-6 mx-auto text-purple-600 dark:text-purple-300">
    ...
    <ul class="flex items-center flex-shrink-0 space-x-6">
      ...
      <!-- Profile menu -->
      <li class="relative">
        <div class="flex items-center justify-center cursor-pointer" @click="toggleProfileMenu" @keydown.escape="closeProfileMenu" aria-label="Account" aria-haspopup="true">
          <div class="align-middle rounded-full focus:shadow-outline-purple focus:outline-none">
            @if (isset(auth()->guard('admin')->user()->avatar))
            <img src="{{ Storage::url(auth()->guard('portal')->user()->avatar) }}" class="user-img object-cover w-6 h-6" alt="">
            @else
            <img src="{{ asset('images/avatar/avatar-default.png') }}" class="user-img object-cover w-6 h-6" alt="user avatar">
            @endif
          </div>
        ...
        <template x-if="isProfileMenuOpen">
          <ul x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @click.away="closeProfileMenu" @keydown.escape="closeProfileMenu" class="absolute right-0 w-56 p-2 mt-2 space-y-2 text-gray-600 bg-white border border-gray-100 rounded-md shadow-md dark:border-gray-700 dark:text-gray-300 dark:bg-gray-700" aria-label="submenu">
            ...
            <li class="flex btn-logout">
              ...
              <form class="logout-form" action="{{ route('doLogout') }}" method="post">
                @csrf
              </form>
            </li>
          </ul>
